...
- Upload the `My Clippings.txt` from your kindle to the `master` branch (replace the previous version) and commit it
- The GitHub action is then triggered and merges new annotations to the `notes` branch
- Check out the `notes` and review your annotations
